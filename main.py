...
async def send_help(message: types.Message):
    help_text = (
        ...
        "• Нажмите кнопку 🔤 *Перевести текст*, чтобы отправить текст для перевода на выбранный язык с голосовым дублированием.\n"
...
async def handle_text(message: types.Message):
    ...
    # Ограничение на длину текста для gTTS (200 символов)
    if len(translated_text) > 200:
        await message.reply("Перевод слишком длинный для голосового сообщения.")
        return
    ...
    voice_file_name = f'voice/voice_{user_id}_{voice_counter[user_id]}.ogg'
